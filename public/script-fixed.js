...
// Counter elements - using safe selectors
...
async function testAttack(attackType, data) {
    ...
    try {
        ...
        try {
            ...
            httpStatus = 'ERROR';
            console.log('WAF test error:', error);
        }
...
function displayResult(result) {
    ...
    if (!resultStatus || !resultCard) return;
    ...
    // Set content safely
    if (resultTimestamp) resultTimestamp.textContent = new Date(result.timestamp).toLocaleString();
    if (resultAttackType) resultAttackType.textContent = result.attackType;
    if (resultPayload) resultPayload.textContent = result.originalPayload || result.submittedPayload;
